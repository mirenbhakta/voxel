...
//
// Rasterizes the [0,8]^3 sub-chunk bounding cube so that the fragment shader
// can start the voxel DDA at the rasterized hull-entry point instead of
// tracing the ray-AABB intersection per pixel from the camera.
//
// SV_VertexID 0..35 indexes into a static 36-vertex cube (6 faces × 2 tris
// × 3 verts). Windings are CCW from outside so that
...
void main(uint vid : SV_VertexID,
    ...
    // Uniform per-cube: is the camera inside (or near-plane-close to) the
    // [0,8]^3 sub-chunk? Evaluated 36 times per draw rather than per-fragment
    // to keep the fragment stage branchless beyond a single select.
    bool inside = all(g_camera.pos > float3(-INSIDE_MARGIN, -INSIDE_MARGIN, -INSIDE_MARGIN))
               && all(g_camera.pos < float3(8.0 + INSIDE_MARGIN, 8.0 + INSIDE_MARGIN, 8.0 + INSIDE_MARGIN));
