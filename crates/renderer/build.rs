...
fn main() {
    ...
    // The generated gpu_constants.hlsl values are sourced from Rust constants
    // in `src/subchunk.rs`; rebuild when that file changes so the shader and
    // Rust-side values stay locked together.
    println!("cargo:rerun-if-changed=src/subchunk.rs");

    // Emit `${OUT_DIR}/shaders/gpu_constants.hlsl` + `${OUT_DIR}/gpu_constants_assert.rs`
    // from a single table. The HLSL header is passed to DXC via an extra `-I`;
    // the Rust assert file is `include!`d from `src/subchunk.rs` so any drift
    // between the values below and the Rust `pub const`s is a compile error.
    emit_gpu_constants(&out_dir, &spv_out_dir);
    ...
    for (src, stage) in collect_entry_points(&shaders_dir) {
        ...
        match &dxc {
            Some(dxc_path) => {
                compile_with_dxc(dxc_path, &src, &out, stage, &shaders_dir, &spv_out_dir)
            }
...
// --- GPU constants header generation ---

/// Shader-visible constants whose numeric values are authored in
/// `src/subchunk.rs`. The entries below MUST match the Rust `pub const`
/// declarations — drift is caught at Rust compile time by the generated
/// `gpu_constants_assert.rs`.
///
/// Fields: (HLSL macro name, Rust const name, numeric literal, HLSL suffix).
/// The HLSL suffix is the type letter (`u` for `uint`). The literal is
/// emitted verbatim into HLSL so hex vs. decimal formatting is preserved.
const GPU_CONSTANTS: &[(&str, &str, &str, &str)] = &[
    ("MATERIAL_DESC_CAPACITY", "MATERIAL_DESC_CAPACITY", "256", "u"),
    (
        "SLOTS_PER_SEGMENT",
...
fn emit_gpu_constants(out_dir: &Path, spv_out_dir: &Path) {
    // HLSL header: dropped into the per-shader include path so entry-point
    // shaders can `#include "gpu_constants.hlsl"` (unqualified) after we
    // extend the DXC include search path to cover `${OUT_DIR}/shaders`.
    let hlsl_path = spv_out_dir.join("gpu_constants.hlsl");
    let mut hlsl = String::new();
    hlsl.push_str(
        "// AUTO-GENERATED by crates/renderer/build.rs — do not edit.\n\
         // Source of truth: crates/renderer/src/subchunk.rs\n\
         //\n\
         // Every value here mirrors a `pub const` in `subchunk.rs`; drift\n\
         // is caught at Rust compile time by `gpu_constants_assert.rs`.\n\
         \n\
         #ifndef RENDERER_GPU_CONSTANTS_HLSL\n\
         #define RENDERER_GPU_CONSTANTS_HLSL\n\
         \n",
    );
    for (hlsl_name, _rust_name, value, suffix) in GPU_CONSTANTS {
        hlsl.push_str(&format!("#define {hlsl_name} {value}{suffix}\n"));
    }
    hlsl.push_str("\n#endif // RENDERER_GPU_CONSTANTS_HLSL\n");
    std::fs::write(&hlsl_path, hlsl)
        .unwrap_or_else(|e| panic!("failed to write {}: {e}", hlsl_path.display()));

    // Rust-side assertions. `include!`d from `src/subchunk.rs` where the
    // authoritative `pub const`s live, turning any drift into a
    // compile-time error pointing at the offending constant.
    let assert_path = out_dir.join("gpu_constants_assert.rs");
    let mut rs = String::new();
    rs.push_str(
        "// AUTO-GENERATED by build.rs — do not edit.\n\
         // Included from src/subchunk.rs to assert Rust consts match the\n\
         // values emitted into shaders/gpu_constants.hlsl.\n\
         \n",
    );
    for (_hlsl_name, rust_name, value, _suffix) in GPU_CONSTANTS {
        // Cast via `as u64` so the assert is agnostic to whether the Rust
        // const is `usize`, `u32`, or `u64` — we only care about the
        // numeric equality.
        rs.push_str(&format!(
            "const _: () = assert!({rust_name} as u64 == {value}u64);\n"
        ));
    }
    std::fs::write(&assert_path, rs)
        .unwrap_or_else(|e| panic!("failed to write {}: {e}", assert_path.display()));
}
...
///
/// Two include paths are supplied: the in-tree `shaders/` directory (hosts
/// `include/*.hlsl`) and the per-build `${OUT_DIR}/shaders/` directory
/// (hosts generated files like `gpu_constants.hlsl`). The generated
/// directory is listed second so an in-tree file of the same name would
/// take precedence — there currently isn't one, but the order makes the
/// intent explicit.
fn compile_with_dxc(
    dxc: &Path,
    src: &Path,
